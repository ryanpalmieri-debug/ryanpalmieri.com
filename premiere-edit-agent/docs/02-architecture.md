...
| 3 | **Sync** | companion (math + FCPXML generation) + panel (import) | inventory → `sync.json` (camera↔WAV pairs, offsets, method, confidence, drift) → generated FCP7 XML creating one **multicam source sequence per camera clip** (`<clip>.movMulticam`: V1 camera, A1–A6 poly-WAV channels at offset, A7–A8 scratch — the editor's real convention, validated against `fixtures/Selects_Sync_Sequence.xml`) plus one `01_STRINGOUT_<SUBJECT>_v###` stringing out that interview's multicams. Failures → REVIEW bin + report, never guessed |
